...
echo ""
echo "检查 CodeBuddy..."
CODEBUDDY_PATH=${CODEBUDDY_PATH:-/root/.npm/node_modules/bin/claude-internal}

if [ ! -f "$CODEBUDDY_PATH" ]; then
    echo "❌ 错误: 未找到 CodeBuddy"
    echo "   路径: $CODEBUDDY_PATH"
    echo ""
    echo "请设置 CODEBUDDY_PATH 环境变量或安装 CodeBuddy"
    exit 1
fi

echo "✅ CodeBuddy: $CODEBUDDY_PATH"
...
echo "企业微信回调服务 V2 配置:"
echo "- 服务端口: $PORT"
echo "- 日志文件: $SCRIPT_DIR/app-v2.log"
echo "- CodeBuddy: $CODEBUDDY_PATH"
echo "============================================================"
echo ""
...
LOG_FILE="$SCRIPT_DIR/app-v2.log"

if [ -f "$PID_FILE" ]; then
    OLD_PID=$(cat "$PID_FILE")
    if ps -p "$OLD_PID" > /dev/null 2>&1; then
        echo "⚠️  服务已在运行 (PID: $OLD_PID)"
        echo "如需重启，请先运行: ./stop-v2.sh"
        exit 1
    else
        rm -f "$PID_FILE"
    fi
...
nohup python3 -m uvicorn app-v2:app --host 0.0.0.0 --port $PORT > "$LOG_FILE" 2>&1 &
APP_PID=$!
