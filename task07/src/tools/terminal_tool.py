        ...
        if command_name not in self.allowed_commands:
            ...
            if os.name == 'nt':
                if command_name == 'pwd':
                    ...
                    # Emulate ls
                    try:
                        target_dir = args[0] if args and not args[0].startswith('-') else '.'
                        if self._is_dangerous_path(target_dir):
                            return f"🚫 危险路径: {target_dir}"
                    ...
                    try:
                        target_file = args[0]
                        if self._is_dangerous_path(target_file):
                            return f"🚫 危险路径: {target_file}"
